...
import java.io.InputStream; // for IOException and Input/OutputStream
...
class HTTPProtocolFactory implements ProtocolFactory {
	...
	private static final String DOCROOTPROP = "DocumentRoot"; // Doc root
																// property
	...
	public Runnable createProtocol(final Socket clntSock, final Logger logger) {
		return new Runnable() {
			public void run() {
				...
				HTTPProtocolFactory
						.handleClient(clntSock, logger, documentRoot);
			}
		};
	}
	...
	private static String getFilename(InputStream in) throws IOException {
		...
		StreamTokenizer tokenizer = new StreamTokenizer(new InputStreamReader(
				in));
		tokenizer.ordinaryChar('/'); // By default the single / character is a
										// comment
		tokenizer.wordChars('!', '~'); // Set these characters in a URL as a
										// word
		tokenizer.nextToken();
		if ((tokenizer.ttype != tokenizer.TT_WORD)
				|| (tokenizer.sval.compareTo("GET") != 0)) {
